...
st.sidebar.write(f"주차장 수 : {len(df)}")
...
if fee_col:
    ...
    if len(cheapest):
        ...
        st.write(cheapest.iloc[0][0])

        st.write("주소")

        st.write(cheapest.iloc[0][address_col])

        st.write("기본요금")

        st.write(f"{cheapest.iloc[0][fee_col]} 원")
...
if lat_col and lng_col:
    ...
    for _, row in df.iterrows():
        ...
        free = "-"
        if free_col:
            free = str(row[free_col])

        operating = "-"
        if time_col:
            operating = str(row[time_col])

        # 마커 색상
        color = "blue"

        if free_col:
            if "무료" in free or "Y" in free or "예" in free:
                color = "green"
